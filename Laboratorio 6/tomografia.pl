...
matrixByRows([],_,[]).
matrixByRows(L,NumCols,[Fila|Matrix]):- length(Fila,NumCols), append(Fila,L1,L),
	matrixByRows(L1, NumCols,Matrix).

declareConstraints(RowSums,ColSums,MatrixByRows,MatrixByColumns):- checkSuma(RowSums,MatrixByRows), checkSuma(ColSums,MatrixByColumns).

checkSuma([],[]):-!.
checkSuma([SumAct|SumVec], [Vec|Matrix]):- sum(Vec, #=, SumAct), checkSuma(SumVec,Matrix).
