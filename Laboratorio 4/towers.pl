...
row(I):- map_size(N,_), between(1,N,I).                         % fila I
col(J):- map_size(_,M), between(1,M,J).                         % columna J
position(I,J):- row(I), col(J).                                 % posicion I-J
village(V):- village(V,_,_,_).                                  % V identificador
numVillages(N):- findall(V,village(V),L), length(L,N).          % Numero de villages
significantVillage(V):- significantVillages(L), member(V,L).    % V es significant

% V tiene una casilla en la fila I
rowVillage(V,I):- village(V,I1,_,S), I2 is I1+S-1, between(I1,I2,I).

% V tiene una casilla en la columna J
colVillage(V,J):- village(V,_,J1,S), J2 is J1+S-1, between(J1,J2,J).

% V tiene una casilla en I-J
posVillage(V,I,J):- rowVillage(V,I), colVillage(V,J).

% V esta vigilado en la posicion I-J
posWatchesVillage(V,I,J):- position(I,J), rowVillage(V,I).
...
satVariable( towerPos(I,J) ):- row(I), col(J).  % torre en posicion I-J
...
writeClauses(MaxNumTowers):-
    eachSignificantExactlyOne(),
    eachVillageAtMostOne(),
    eachRowAtMostOne(),
    eachColAtMostOne(),
    eachVillageWatched(),
    exactlyMaxTowers(MaxNumTowers),
    torreImplicaPosition(),
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, !.




% para todas las significantVillages (V), de todas las casillas tiene que haber exactamente 1 con una torre
eachSignificantExactlyOne():- significantVillage(V), findall(towerPos(I,J),
    (rowVillage(V,I),colVillage(V,J)), %fila I, columna J tal que I,J es casilla de V
    Lits), exactly(1,Lits),fail.
eachSignificantExactlyOne().

% para todas las villages (V), para todas sus casillas al menos 1 tiene que estar vigilada por una torre -> al menos 1 torre en las filas o columnas de V
eachVillageWatched():- village(V), findall(towerPos(I,J),
    (village(V2), posVillage(V2,I,J), posWatchesVillage(V,I,J)),
    %towerPos tal que I y J pertenecen a las I,J de la village y hay una village2 que la vigila
    Lits), atLeast(1,Lits),fail.
eachVillageWatched().


%dada una village, debe haber una torre en una de las filas o columnas -> 
%eachVillageWatched():-

% para todas las villages (V), para todas sus casillas 
eachVillageAtMostOne():-village(V), findall(towerPos(I,J),
    (posVillage(V,I,J)),
    Lits), atMost(1,Lits),fail.
eachVillageAtMostOne().

%cada fila como mucho una torre
eachRowAtMostOne():- row(I), findall(towerPos(I,J),col(J), Rows), atMost(1,Rows), fail.
eachRowAtMostOne().

%cada columna como mucho una torre
eachColAtMostOne():- col(J), findall(towerPos(I,J),row(I), Cols), atMost(1,Cols), fail.
eachColAtMostOne().


%como maximo MaxNumTowers colocadas
exactlyMaxTowers(MaxNumTowers):- findall(hasTower(V), village(V), Towers), atMost(MaxNumTowers,Towers),!.

% si existe (hasTower(V)) es porque hay una casilla de V con towerPos(I,J) --> hasTower -> towerPos -> [hasTower v -towerPos]
torreImplicaPosition():- village(V), posVillage(V,I,J), writeClause([hasTower(V), -towerPos(I,J)]), fail.
torreImplicaPosition().
...
%calcula el numero de torres usadas -> numero de posiciones (i,j) tales que i es fila y j columna y hay una torre en M
costOfThisSolution(M,Cost):- findall(position(I,J),
(row(I), col(J), member(towerPos(I,J),M)), L),
