...
task(T):-          gangstersNeeded(T,_).    %tarea T
needed(T,H,N):-    gangstersNeeded(T,L), nth1(H,L,N). %para al tarea T a la hora H, se necesitan N gangsters
gangster(G):-      gangsters(L), member(G,L).   %gangster G
hour(H):-          between(1,72,H). %hora H
blocked(G,H):-     notAvailable(G,L), member(H,L). % gangster G no disponible a la hora H
available(G,H):-   hour(H), gangster(G), \+blocked(G,H). % gangster G disponible a la hora H
...
%gangster G a la hora H esta ocupado y G pertenece a los Gs necesarios para realizar T a esa H
satVariable( does(G,T,H) ):- gangster(G), hour(H), task(T).
...
writeClauses(MaxConsecutiveHours):-
    eachHourAtMostOneTask,
    noConsecutiveDifferentTasks,
    allTasksDone,
    notAvailableGangsters,
    atMostConsecutiveHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


%dado un gangster y una hora, puede hacer como maximo una task
eachHourAtMostOneTask:- gangster(G), hour(H), findall( does(G,T,H), task(T), L), atMost(1,L),
fail.
eachHourAtMostOneTask.

%dado un gangster y sus horas disponibles, si tiene una tarea a una hora no puede tener una tarea diferente a la siguiente
noConsecutiveDifferentTasks:- gangster(G), available(G,H1), available(G,H2), H2 is H1+1, task(T1), task(T2), T1\=T2, writeClause([-does(G,T1,H1), -does(G,T2,H2)]),fail.
noConsecutiveDifferentTasks.

%dada una tarea y su hora, se deben asignar los N gangsters que deben hacerla -> exactly N clausulas does(G,H,T)
allTasksDone:- needed(T,H,N), findall(does(G,T,H), gangster(G), L), exactly(N,L) ,fail.
allTasksDone.

%dado un gangster y sus notAvailable horas, no puede tener tasks en esas horas
notAvailableGangsters:- blocked(G,H), task(T), writeClause([-does(G,T,H)]), fail.
notAvailableGangsters.

%dado un gangster, una tarea y una hora (H1), encontramos todas las ocupaciones (does) con hora entre H1 y HMax. Negamos todos los literales y tiene que haber al menos 1 de ellos (de todas las horas consecutivas, en al menos 1 descansara (not does))
atMostConsecutiveHours(MaxHours):- gangster(G), task(T),
    hour(H1), hour(HMax), HMax is H1+MaxHours, findall(does(G,T,H), between(H1,HMax,H), Does),
    negateAll(Does,NDoes), atLeast(1,NDoes) ,fail.
...
costOfThisSolution(M,Cost):- between(0,71,N),
   Cost is 72-N,
   gangster(G),
